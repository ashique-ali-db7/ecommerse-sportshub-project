...
module.exports = {
  ...
  checkLogin: (data) => {
    return new Promise(async (resolve, reject) => {
      ...
      let user = await db
        .get()
        .collection(collections.USERS_DETAILS_COLLECTION)
        .findOne({ emailaddress: data.emailaddress });
  ...
  adminLogin: (data) => {
    return new Promise(async (resolve, reject) => {
      ...
      let user = await db
        .get()
        .collection(collections.ADMIN_DETAILS_COLLECTION)
        .findOne({
          $and: [
            { emailaddres: data.emailaddress },
  ...
  blockUser: (phonenumber) => {
    return new Promise(async (resolve, reject) => {
      await db
        .get()
        .collection(collections.USERS_DETAILS_COLLECTION)
        .updateOne({ phonenumber: phonenumber }, { $set: { blocked: true } })
        .then((status) => {
          resolve();
        });
    });
  ...
  unblockUser: (phonenumber) => {
    return new Promise(async (resolve, reject) => {
      await db
        .get()
        .collection(collections.USERS_DETAILS_COLLECTION)
        .updateOne({ phonenumber: phonenumber }, { $set: { blocked: false } })
        .then((status) => {
          resolve();
        });
    });
        ...
        let exist = await db
          ...
          .findOne({
            $and: [
              {
                _id: objectId(userId),
                address: { $elemMatch: { defaultaddress: "on" } },
              },
            ],
          });
        if (exist) {
  ...
  editotheraddress: (otheraddressid, userId) => {
    return new Promise(async (resolve, reject) => {
      let getSingleOtherAddressForEdit = await db
        .get()
        .collection(collections.USERS_DETAILS_COLLECTION)
        .aggregate([
          { $match: { _id: objectId(userId) } },
            ...
            .updateOne(
              {
                $and: [
                  {
                    _id: objectId(oneproduct.item),
                    instock: { $elemMatch: { size: oneproduct.size } },
                  },
                ],
              },
    ...
    return new Promise(async (resolve, reject) => {
      let product = await db
        ...
        .findOne({
          ...
          products: { $elemMatch: { item: objectId(proId), size: size } },
        });
  ...
  getorderedproditdetils: (orderid) => {
    return new Promise(async (resolve, reject) => {
      let vieworderproductdetails = await db
        .get()
        .collection(collections.ORDER_DETAILS_COLLECTION)
        .aggregate([
          {
  ...
  saveProfile: (data, userId) => {
    return new Promise((resolve, reject) => {
      db.get()
        .collection(collections.USERS_DETAILS_COLLECTION)
        .update({ _id: objectId(userId) }, { $set: { profile: data } })
        .then(async (response) => {
